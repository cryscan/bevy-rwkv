...
fn layer_norm(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
    ...
    sum[index] = 0.0;
    for (var i = index; i < num_embd; i += BLOCK_SIZE) {
        let value = x[num_embd * token + i];
        sum[index] += value;
    ...
    if index == 0u {
        mean = sum[0] / f32(num_embd);
        std = sqrt(sum_squared[0] / f32(num_embd) - mean * mean);
    }
    workgroupBarrier();

    for (var i = index; i < num_embd; i += BLOCK_SIZE) {
        let value = (x[num_embd * token + i] - mean) / std;
        output[num_embd * token + i] = value * w[i] + b[i];
    }
}